#!/usr/bin/env python
# -*- encoding: utf-8 -*-
## This python file derives https://github.com/tobegit3hub/tensorflow_template_application/blob/master/sparse_classifier.py
## Thx to Mr.Tobe for the great work. New modification here mainly focus on export of freezed graph for java inference.
import datetime
import sys,json
import logging
import math
...
flags.DEFINE_string("train_tfrecords_file", "/data/jhluo/ctr-liebiao/20171205/20180111.sample.train.libsvm.tfrecords",
                    #"./data/cod-rna/cod-rna.libsvm.tfrecords",
                    "The glob pattern of train TFRecords files")
flags.DEFINE_string("validate_tfrecords_file",
                    #"./data/cod-rna/cod-rna-test.libsvm.tfrecords",
                    "/data/jhluo/ctr-liebiao/20171205/20180111.sample.train.libsvm.tfrecords",
                    "The glob pattern of validate TFRecords files")
flags.DEFINE_integer("feature_size", 128, "Number of feature size")
...
flags.DEFINE_integer("validate_batch_size", 10240,
...
flags.DEFINE_string("model_network", "32 16 8", "The neural network of model")
...
flags.DEFINE_string("mode", "train", "Support train, export, inference, inference_with_freeze")
...
flags.DEFINE_string("freezed_model_path", "./sparse_model/dnn_graph.db", "The path of the freezed model")
flags.DEFINE_integer("model_version", 1, "The version of the model")
flags.DEFINE_string("inference_test_file", 
                    "/data/jhluo/ctr-liebiao/20171205/20180111.sample.test.libsvm.1w",
                    #"./data/cod-rna/cod-rna-test.libsvm",
...
flags.DEFINE_boolean("continue_train", False, "whether continue to train using last checkpoint model")

def main():
  """
  if len(sys.argv) < 2:
    print "provide task mode: train, export, inference, inference_with_freeze"
    sys.exit(-1)
  FLAGS.mode=sys.argv[1]
  
  if FLAGS.mode == "train":
    FLAGS.feature_size = data_input(FLAGS.inference_test_file).get_feature_num()
  """
  # Get hyperparameters
...
# Read TFRecords file for validation
  ...
  validate_batch_sample_ids = validate_features["sample_id"]
  ...
  sparse_index_1dim = tf.placeholder(tf.int64, [None], name="sparse_index")
  sparse_index = tf.reshape(sparse_index_1dim, [tf.size(sparse_values),2], name="conv_sparse_index")
  #sparse_index = tf.placeholder(tf.int64, [None, 2], name="sparse_index")
  ...
  #with inference_values = sparse_placeholder, feed with (2d sparse_ids, sparse_values)

  inference_logits = inference(inference_ids, inference_values, False)
  inference_softmax = tf.nn.softmax(inference_logits, name="output_probs")
  ...
  with tf.Session() as sess:
    ...
    if MODE == "train":
      ...
      try:
        while not coord.should_stop():
          if FLAGS.benchmark_mode:
            ...
            """
            debug_batch_ids, debug_batch_values = sess.run([train_softmax, batch_values])
            
            print "debug_batch_ids:"
            print debug_batch_ids
            print "debug_batch_values:"
            print debug_batch_values
            """
            # Print state while training
            if step % FLAGS.steps_to_validate == 0:
              loss_value, train_accuracy_value, train_auc_value, validate_accuracy_value, auc_value, summary_value, debug_train_softmax = sess.run(
                  [
                      loss, train_accuracy, train_auc, validate_accuracy,
                      validate_auc, summary_op, train_softmax
                  ])
              end_time = datetime.datetime.now()

              """debug train results"""
              np.savetxt(FLAGS.inference_result_file+".train.txt", debug_train_softmax, delimiter="\t", fmt="%10.6f")
              """debug validate results"""
              sample_id, debug_validate_softmax = sess.run([validate_batch_sample_ids, validate_softmax])
              #print [sample_id, debug_validate_softmax]
              
              np.savetxt(FLAGS.inference_result_file+".validate.txt", debug_validate_softmax, delimiter="\t", fmt="%10.6f")
      ...
      inference_test_file_name = FLAGS.inference_test_file #"./data/a8a/a8a_train.libsvm"
      labels = []
      feature_ids = []
      feature_values = []
      feature_index = []
      ...
      np.savetxt(inference_result_file_name, prediction_softmax, delimiter="\t",fmt='%10.6f')
      logging.info(
...
def export_model(sess, saver, signature, model_path, model_version):
  ...
  try:
    ...
    #export the freeze model for inference by java program. refer to DNNModelImp.java
    graph = tf.graph_util.convert_variables_to_constants(sess, sess.graph_def,["sparse_index", "sparse_ids", "sparse_values", "sparse_shape", "output_probs","conv_sparse_index"])
    tf.train.write_graph(graph, ".", FLAGS.freezed_model_path, as_text=False)
    tf.train.write_graph(graph, ".", FLAGS.freezed_model_path+".txt", as_text=True)
